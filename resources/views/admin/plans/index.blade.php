			...
			<div class="card" style="height: 473px;">
				...
				<div class="card-body">
					...
					@if (Auth::user()->type == 2)
						...
						@if($showbuy == "no")
						@if ($pdata->price > 0)
						<a href="{{ URL::to('/vendor/plans/'.$pdata->id)}}" class="btn btn-raised btn-success btn-min-width mr-1">{{trans('labels.buy_now')}}</a>
						@endif
						<span class="text-danger">
							{{trans('labels.current_plan')}} ({{trans('labels.expired')}} {{date('d/m/Y', strtotime($exdate))}})
						</span>
						@else
						<span class="text-success">
							{{trans('labels.current_plan')}} ({{trans('labels.expired_on')}} {{date('d/m/Y', strtotime($exdate))}})
						</span>
						@endif
