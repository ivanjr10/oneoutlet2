<form class="form form-horizontal form-bordered" action="@if (Auth::user()->type == 1) {{URL::to('admin/settings/update')}} @endif @if (Auth::user()->type == 2) {{URL::to('vendor/settings/update')}} @endif" method="POST" enctype="multipart/form-data">
   ...
   @if (Auth::user()->type == 1 && App\Models\SystemAddons::where('unique_identifier', 'custom_domain')->first() != null && App\Models\SystemAddons::where('unique_identifier', 'custom_domain')->first()->activated)
   <div class="card" id="custom_domain">
      <div class="card-body">
         <div class="px-3">
            <div class="form-body">
               <h4 class="form-section"><i class="fa fa-bar-chart"></i> {{trans('labels.custom_domain')}} </h4>
               <div class="form-group row">
                  <label class="col-md-2 label-control" for="cname_section_title">{{trans('labels.cname_section_title')}}</label>
                  <div class="col-md-10">
                     <input type="text" name="cname_section_title" class="form-control" placeholder="{{trans('labels.cname_section_title')}}" value="{{@$settingdata->cname_title}}">
                     @error('cname_section_title')<span class="text-danger" id="meta_title_error">{{ $message }}</span>@enderror

                  </div>
               </div>
               <div class="form-group row">
                  <label class="col-md-2 label-control" for="cname_section_text">{{trans('labels.cname_section_text')}}</label>
                  <div class="col-md-10">
                     <textarea class="form-control" name="cname_section_text" placeholder="{{trans('labels.cname_section_text')}}">{{@$settingdata->cname_text}}</textarea>
                     @error('cname_section_text')<span class="text-danger" id="cname_section_text_error">{{ $message }}</span>@enderror
      ...
      <div class="card-body">
         <div class="px-3">
            <div class="form-body">
               ...
               @if (App\Models\SystemAddons::where('unique_identifier', 'language')->first() != null && App\Models\SystemAddons::where('unique_identifier', 'language')->first()->activated)
               <div class="form-group row">
                  <label class="col-md-2 label-control" for="language">{{trans('labels.language')}} </label>
                  <div class="col-md-10">
                     <select class="form-control" name="language">
                        @foreach($languages as $rdata)
                        <option value="{{$rdata->code}}" {{ ( $rdata->code == @$settingdata->language) ? 'selected' : '' }}>{{$rdata->name}}</option>
                        @endforeach
                     </select>
               ...
               <div class="form-group row">
                  <label class="col-md-2 label-control" for="show_language">{{trans('labels.show_language')}}</label>
                  <div class="col-md-10">
                     <div class="form-check form-check-inline">
                        <input class="form-check-input" type="radio" name="show_language" id="1" value="1" @if(@$settingdata->show_language == "1") checked="checked" @endif>
                        <label class="form-check-label" for="1">{{trans('labels.yes')}}</label>
                     </div>
                     <div class="form-check form-check-inline">
                        <input class="form-check-input" type="radio" name="show_language" id="2" value="2" @if(@$settingdata->show_language == "2") checked="checked" @endif>
                        <label class="form-check-label" for="2">{{trans('labels.no')}}</label>
                     </div>
                     @error('show_language')<span class="text-danger" id="show_language_error">{{ $message }}</span>@enderror
         ...
         <div class="px-3">
            <div class="form-body">
               ...
               <div class="card-block">
                  <h5 class="text-center">Order variable</h5>
                  <hr>
                  <div class="row d-flex">
                     <div class="col align-self-center">
                        <p class="mb-1">Número do pedido: <span class="pull-right text-primary">{order_no}</span></p>
                        <p class="mb-1">Variável do item: <span class="pull-right text-primary">{item_variable}</span></p>
                        <p class="mb-1">Taxa de entrega: <span class="pull-right text-primary">{delivery_charge}</span></p>
                        <p class="mb-1">Valor do desconto: <span class="pull-right text-primary">{discount_amount}</span></p>
                        <p class="mb-1">Total geral: <span class="pull-right text-primary">{grand_total}</span></p>
                        <p class="mb-1">Observações: <span class="pull-right text-primary">{notes}</span></p>
                        <p class="mb-1">Nome do cliente: <span class="pull-right text-primary">{customer_name}</span></p>
                        <p class="mb-1">Telefone do cliente: <span class="pull-right text-primary">{customer_mobile}</span></p>
                        <p class="mb-1">Tipo de pagamento: <span class="pull-right text-primary">{payment_type}</span></p>
                        <p class="mb-1">Nome da loja: <span class="pull-right text-primary">{store_name}</span></p>
                        <p class="mb-1">URL de rastreamento: <span class="pull-right text-primary">{track_order_url}</span></p>
                        <p class="mb-1">URL da loja: <span class="pull-right text-primary">{store_url}</span></p>
                     </div>
                     <div class="col align-self-center">
                        <p class="mb-1">Endereço: <span class="pull-right text-primary">{address}</span></p>
                        <p class="mb-1">Nº: <span class="pull-right text-primary">{building}</span></p>
                        <p class="mb-1">Ponto de referência: <span class="pull-right text-primary">{landmark}</span></p>
                        <p class="mb-1">Tipo de entrega: <span class="pull-right text-primary">{delivery_type}</span></p>
                        <hr>
                        <h5 class="text-center">{{trans('labels.item_variable')}}</h5>
                        <hr>
                        <p class="mb-1">Nome do item: <span class="pull-right text-primary">{item_name}</span></p>
                        <p class="mb-1">Qtd: <span class="pull-right text-primary">{qty}</span></p>
                        <p class="mb-1">Variantes: <span class="pull-right text-primary">{variantsdata}</span></p>
                        <p class="mb-1">Preço do item: <span class="pull-right text-primary">{item_price}</span></p>
