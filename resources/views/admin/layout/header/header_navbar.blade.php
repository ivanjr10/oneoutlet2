   ...
   <div class="container-fluid">
      <div class="navbar-header">
         @if (App\Models\SystemAddons::where('unique_identifier', 'language')->first() != null && App\Models\SystemAddons::where('unique_identifier', 'language')->first()->activated)
            @if(Helper::webinfo(Auth::user()->id)->show_language == 1)
            <div class="row">
               <div class="col-lg-12">
                  <select class="form-control changeLang">
                     @foreach(Helper::listoflanguage() as $languagelist)
                        <option value="{{$languagelist->code}}" {{ ( session()->get('language-admin') == $languagelist->code) ? 'selected' : '' }}>{{$languagelist->name}}</option>
                     @endforeach
                  </select>
               </div>
            </div>
