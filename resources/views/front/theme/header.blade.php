                ...
                <div class="d-flex justify-content-between">
                    <div class="d-flex align-item-center">
                        <a class="navbar-brand mx-3" href="{{ URL::to($getrestaurant->slug) }}">
                            <img src="{{ Helper::webinfo(@$getrestaurant->id)->image }}" alt="">
                        </a>
                        <h2 class="text-white m-0 logo-text">{{ Helper::getrestaurant($getrestaurant->slug)->name }}</h2>
                    </div>
                    ...
                    @if (App\Models\SystemAddons::where('unique_identifier', 'language')->first() != null && App\Models\SystemAddons::where('unique_identifier', 'language')->first()->activated)
                        @if(Helper::webinfo($getrestaurant->id)->show_language == 1)
                        <div class="row">
                            <div class="col-lg-12">
                                <select class="form-control changeLang">
                                    @foreach(Helper::listoflanguage() as $languagelist)
                                        <option value="{{$languagelist->code}}" {{ ( session()->get('language') == $languagelist->code) ? 'selected' : '' }}>{{$languagelist->name}}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
