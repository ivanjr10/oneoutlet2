...

{
    ...
    public static function is_store_closed($restaurant)
    {
        ...
        $todaydata = Timing::where('restaurant',@$restaurant)->where('day',date("l",strtotime(date('d-m-Y'))))->first();
            ...
            $current_time = \DateTime::createFromFormat('G:i', date("H:i"));
            $open_time = \DateTime::createFromFormat('G:i', $todaydata->open_time);
            $close_time = \DateTime::createFromFormat('G:i', $todaydata->close_time);
    ...
    public static function language($restaurant)
    {
        $settingdata = Settings::where('restaurant', $restaurant)->select('language')->first();
        $layout = Languages::select('layout')->where('code', session()->get('language'))->first();

        if (session()->has('language')) {
            App::setLocale(session()->get('language'));
            session()->put('direction', $layout->layout);
        } else {
            Session::put('language', @$settingdata->language);
            App::setLocale(@$settingdata->language);
